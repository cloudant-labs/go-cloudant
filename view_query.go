...
func NewViewQuery() *ViewQuery {
	return &ViewQuery{Values: url.Values{}}
}
...
func (q *ViewQuery) EndKeyDocID(endKeyDocID string) *ViewQuery {
	if endKeyDocID != "" {
		q.Values.Set("endkey_docid", fmt.Sprintf("\"%s\"", endKeyDocID))
	}
	return q
}
...
func (q *ViewQuery) GroupLevel(groupLevel int) *ViewQuery {
	if groupLevel > 0 {
		q.Values.Set("group_level", strconv.Itoa(groupLevel))
	}
	return q
}
...
func (q *ViewQuery) Key(key string) *ViewQuery {
	if key != "" {
		q.Values.Set("key", fmt.Sprintf("\"%s\"", key))
	}
	return q
}
...
func (q *ViewQuery) StartKeyDocID(startKeyDocID string) *ViewQuery {
	if startKeyDocID != "" {
		q.Values.Set("startkey_docid", fmt.Sprintf("\"%s\"", startKeyDocID))
	}
	return q
}
